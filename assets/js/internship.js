...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (cyberHeader && !document.getElementById('cyber-floating-styles')) {
    const styleEl = document.createElement('style');
    styleEl.id = 'cyber-floating-styles';
    styleEl.innerHTML = `
      .cyber-header {
      ...
      html.dark .cyber-header.nav-float,
      body.dark .cyber-header.nav-float,
      html.dark-theme .cyber-header.nav-float {
    ...
    window.addEventListener('scroll', () => {
      if (window.scrollY > 40) {
        cyberHeader.classList.add('nav-float');
      } else {
        cyberHeader.classList.remove('nav-float');
      }
    }, { passive: true });
  }
